...
function ScorePageContent({ gameId, n }: { gameId: string; n: string }) {
  const handNumber = parseInt(n)
  const router = useRouter()
  const searchParams = useSearchParams()

  const trumpSuit = searchParams.get('suit') as TrumpSuit
  const kraakTypeStr = searchParams.get('kraak') as KraakType ?? 'geen'
  ...
  async function handleSave() {
    ...
    try {
      ...
      // team_a seats 1&3, team_b seats 2&4
...
export default function ScorePage({ params }: { params: Promise<{ id: string; n: string }> }) {
  const { id, n } = use(params)
  return (
    <Suspense>
      <ScorePageContent gameId={id} n={n} />
    </Suspense>
  )
}
