...
export default function StatsScreen() {
  ...
  useEffect(() => {
    async function load() {
      const [st, ds, gs, ses] = await Promise.all([
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <main className="px-4 py-4 space-y-5 max-w-lg mx-auto">
        ...
        {/* Ranglijst — klikbaar naar speler-detail */}
        <section>
        ...
        {duos.length > 0 && (
          <section>
            ...
            <p className="text-xs text-gray-600 mb-3">Gesorteerd op gemiddelde punten per handje samen</p>
            <div className="space-y-2">
              {duos.map((duo, i) => (
                <div key={`${duo.playerAId}-${duo.playerBId}`} className="bg-gray-900 rounded-xl p-4 border border-gray-800">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="font-mono font-bold">{duo.avgPoints} <span className="text-xs text-gray-500 font-normal">gem</span></span>
                  </div>

                  <div className="grid grid-cols-4 gap-2 text-xs">
                    <div className="text-center">
                      <p className="text-white font-semibold">{duo.handsPlayed}</p>
                      <p className="text-gray-500">Samen</p>
                    </div>
                    <div className="text-center">
