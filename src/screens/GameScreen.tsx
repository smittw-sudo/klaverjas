...
export default function GameScreen() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [gameRes, gpRes, handsRes] = await Promise.all([
        ...
        supabase.from('game_players').select('*, player:players(*)').eq('game_id', gameId!).order('seat_position'),
  ...
  if (!game) return <div className="min-h-screen flex items-center justify-center text-gray-400">Potje niet gevonden.</div>
